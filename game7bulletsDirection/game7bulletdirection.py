...
while lives > 0:
    for event in pygame.event.get():
        ...
        # add mouse event https://www.pygame.org/docs/ref/mouse.html
            ...
            if pygame.mouse.get_pressed()[0]:
                ...
                # if -pi/2 <= angle <= pi/2:
                #     add_bullets(1, player.rect.midright, angle)
                # else:
                #     add_bullets(1, player.rect.midleft, angle)
                # player.image = player.rot_center(0)
    ...
    # print(direction)
    # enemies.update()
    player.update(angle * 180/ pi) # convert angle in degrees as the rotation function accepts only values in degrees
    bullets.update()
